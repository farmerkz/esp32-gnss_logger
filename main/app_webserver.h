 ...
 * @note Вызывать ТОЛЬКО из контекста FreeRTOS-задачи (НЕ из event handler).
 *       httpd_stop() является блокирующей операцией.
 */
void app_webserver_stop(void);

/**
 * @brief Асинхронный запрос на остановку веб-сервера.
 *
 * Безопасно вызывается из любого контекста, включая WiFi event task.
 * Устанавливает внутренний флаг — фактическая остановка произойдёт
 * при следующем вызове app_webserver_process_stop() из задачи.
 */
void app_webserver_request_stop(void);

/**
 * @brief Обработка отложенного запроса на остановку веб-сервера.
 *
 * Должна вызываться периодически из контекста FreeRTOS-задачи (например, из app_ftp_task).
 * Если ранее был вызван app_webserver_request_stop(), выполняет httpd_stop()
 * и сбрасывает флаг. В противном случае возвращается немедленно.
 */
