...
static void app_wigle_task(void *pvParameters)
{
    ...
    for (;;) {
        ...
        if (app_gnss_get_latest_pvt(&pvt) && app_gnss_is_fix_valid()) {
            ...
            /* Флаг: воспроизвести FILE_ROTATE ПОСЛЕ освобождения мьютекса (R-3) */
            ...
            if (s_current_wifi_file != NULL) {
                ...
                fflush(s_current_wifi_file);
                ESP_LOGD(TAG, "Written %d APs to Wigle CSV", ap_count);

                /* Ротация при превышении лимита размера файла */
                ...
                if (current_size >= (long)g_app_config.filesize_limit) {
                    ...
                    ESP_LOGI(TAG, "Wigle CSV reached size limit. Rotated to ready: %s", ready_path);
            ...
            /* R-3: воспроизводим звук ПОСЛЕ освобождения g_sd_mutex */
            if (play_rotate) {
